...
**정족수:** 투표가 통과하려면 DYDX 토큰의 최소 찬성 정족수를 달성해야 합니다. 정족수의 목적은 충분한 유권자 참여가 이루어진 경우에만 제안이 통과되도록 보장하는 것입니다.

**에포크:** 다른 모든 계약은 에포크라고 하는 28일 주기로 작동합니다.

**실행 유예 기간:** 투표 후 DIP 제안이 실행 가능해지는 기간으로, 이 기간 내에 반드시 실행되어야 합니다.

**거버넌스 전략 V2 계약**: 제안 및 투표에 대한 사용자의 상대적인 권한을 측정하는 로직이 포함되어 있습니다.

**거버넌스 계약**: 제안을 추적하고 타임락 스마트 계약을 통해 제안을 실행할 수 있습니다.

**롱 타임락 실행자 :** 롱 타임락 실행자는 일반적으로 거버넌스 합의에 영향을 미치는 프로토콜 일부를 변경하는 제안을 실행할 수 있습니다.
...
**안전 풀:** 프로토콜을 지급 불능 상태로부터 보호하는 역할을 하는 요소입니다.

**스테이킹 dYdX 계약:** DYDX 토큰 스테이킹, 포지션 토큰화와 보상을 받기 위한 로직이 포함됩니다.

**단기 타임락 실행자:** 보상 및 인센티브 계약, 또는 신속한 개입이 필요한 커뮤니티 트레저리를 일반적으로 변경하는 제안을 실행할 수 있습니다.

**Starkware 실행자:** 현재 Starkware의 개입이 필요한 프로토콜 중 일부를 일반적으로 변경하는 제안을 실행할 수 있습니다.
...
**타임락 지연:** DIP 제안이 통과되어 대기열에 추가된 후 실행되기까지의 지연 시간입니다.

**제안 임계값:** 무분별한 스팸 제안이 생성되는 시스템을 방지하기 위해 제안 임계값은 제안을 만들기 전에 주소가 특정 투표 수를 보유할 것을 요구합니다.
...
**제안 임계값**: DIP 제안을 생성하기 위해 보유하거나 위임해야 하는 최소 토큰 수입니다.

**투표 격차:** DIP 제안이 통과하는 데 필요한 찬성 및 반대 표 간의 격차를 말합니다.
