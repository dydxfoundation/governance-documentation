...
7.5% \(`75,000,000 DYDX`\) to liquidity providers based on a formula rewarding a combination of uptime, two-sided depth, bid-ask spreads, and the number of markets supported.

**Objectives**

* Improve 2-sided liquidity and programmatically reward liquidity providers
...
Orders below a certain **minimum depth** \(size\) \($$MinSize$$\) per market are excluded, and orders over a certain **maximum spread** \(mid-market spread\) \($$MaxSpread$$\) market are excluded as well.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td style="text-align:left">
        <p>Assume a liquidity provider has multiple open bid orders (0.1 BTC at $29,900,
          5 BTC at $29,800, 10 BTC at $29,500) on the BTC-USD order book and BTC
          is currently at $30,000 (based on mid-market). Assume TargetDepth is $5000
          and TargetSpread vs. mid-market is $150 (in USD terms).
          <br />
        </p>
        <p></p>
        <p>
    ...
    <tr>
      ...
      <td style="text-align:left">
        <p>Assume a liquidity provider has multiple open ask orders (0.1 BTC at $30,100,
          5 BTC at $31,200, 10 BTC at $31,300) on the BTC-USD order book and BTC
          is currently trading at $30,000 (based on mid-market). Assume TargetDepth
          is $5000 and TargetSpread vs. mid-market is $150 (in USD terms).</p>
        <p></p>
        <p></p>
        <p>
...
Each perpetual market will have its own rewards pool that will be weighted differently. The initial set of weights applied to each market is as follows:
...
All liquidity providers who have achieved a minimum of 5% of maker volume on dYdX Layer 2 perpetuals in the prior epoch are eligible to receive DYDX as rewards in a given epoch.

For Epoch 0, dYdX Trading will run the figures to determine which market makers are eligible. dYdX Trading will determine which market makers reach this threshold.
...
Earned DYDX tokens via the Trading Rewards will be transferable `7 days` \(**Waiting Period**\) after the end of each epoch.

### How do we define and measure two-sided depth, bid-ask spread, and uptime?

**Two-sided depth** 

A two-sided liquidity provider is a firm or individual who actively quotes two-sided markets on dYdX, providing bids and asks for a given market. They provide liquidity to the decentralized exchange overall.

For instance, a market maker in the BTC-USD market may provide a quote of $30,000-$30,100, 10x50. This means that they bid \(they will buy\) 10 BTC for $30,000 and also offer \(they will sell\) 50 BTC at $30,100. Other market participants may then buy \(lift the offer\) from the market maker at $30,100 or sell to them \(hit the bid\) at $30,000.
...
Liquidity provider uptime is critical for markets, especially in periods of high volatility. By applying an exponent of 5 to $$Uptime_{epoch}$$ is as an input to the $$Q_{FINAL}$$, the rewards are skewed towards liquidity providers who maintain 2-sided liquidity constantly. In other words, a liquidity provider who provides uptime 99% of the time is exponentially more valuable than a market maker who provides 90% uptime.

Uptime is defined as the percentage of time orders are in a given market providing liquidity on a minute-by-minute basis \(with randomized sampling\). Uptime excludes periods of time when the exchange faces outages. There may be edge cases where the exchange is slow or not accepting orders \(but is not an outage\)—in which case the above would not apply \(but that would be considered a bug and all market makers would be similarly affected, as with outages\).

### How do we define maximum spreads per market?

No $$Q_{BID}$$ is or $$Q_{ASK}$$is will be generated when the Spread is above a given market’s $$MaxSpread$$.
...
### How do we define the minimum size per market?

No $$Q_{BID}$$ is or $$Q_{ASK}$$is will be generated when the Size is below a given market’s $$MinSize$$.

The initial Min Sizes are as follows:

| **Market** | **Min Size \(Bid and Ask\)** |
| :--- | :--- |
| BTC-USD | $5000 |
| ETH-USD | $5000 |
