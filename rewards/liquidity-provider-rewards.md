...
토큰 공급의 **3.2%**(`31,643,838 $ethDYDX`)는 메이커 거래량, 업타임, 양방향 호가, 매수-매도 호가, 지원 시장 수를 조합하여 보상하는 공식을 기반으로 유동성 공급자(LP)에게 배분되어 할당됩니다. 처음 토큰 공급량의 **7.5%**(`75,000,000 $ethDYDX`)는 유동성 공급자에게 보상으로 돌아갑니다.
...
시장 유동성을 장려하기 위해 $ethDYDX는 dYdX v3에서의 시장 참여, 마켓 메이커 거래량, 양방향 호가, 스프레드(미드마켓 대비), 업타임을 보상하는 공식을 기반으로 유동성 공급자에게 배포될 것입니다. 모든 이더리움 주소는 해당하는 보상을 받을 수 있으며, 이전 에폭상에서 메이커 거래량의 0.25%라는 최소 메이커 거래량 임계값을 조건으로 합니다. $ethDYDX는 5년 동안 28일 에폭 단위로 분배될 예정이며, 그 어떠한 베스팅이나 락업도 적용되지 않습니다.
