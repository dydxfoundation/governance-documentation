...
* **50,0%** (`500 000 000 $ethDYDX`) распределяются между сообществом следующим образом:
  ...
  * **7,5%** (`75 000 000 $ethDYDX`) — прошлым пользователям, которые завершили определенные вехи торговли в протоколе уровня 2 (награды за ретроактивный майнинг),
...
* **50,0%** (`500 000 000 $ethDYDX`) распределяются между сообществом следующим образом:
  * **14,5%** (`144 693 506 $ethDYDX`) по [формуле расчета наград за торговлю](https://docs.dydx.community/dydx-governance/rewards/trading-rewards),
    ...
    * В рамках [ППУ 29](https://dydx.community/dashboard/proposal/16) сообщество проголосовало за сокращение вознаграждения за торговлю на ⅓ с эпохи 30-32 на dYdX v3 до следующих значений:
      ...
      * Эпоха 32: 0 $ethDYDX
  * **5,0%** (`50 309 197 $ethDYDX`) — прошлым пользователям, которые завершили определенные вехи торговли в протоколе dYdX уровня 2 (награды за ретроактивный майнинг),
...
<summary>Может ли распределение $ethDYDX измениться?</summary>
...
25 января 2023 года dYdX Foundation [объявил](https://www.dydx.foundation/blog/lock-up-extension) о внесении поправки о переносе даты первоначального выпуска токенов инвестора $ethDYDX на 1 декабря 2023 года. Поправка не изменила скользящий график расписания разблокировки после даты изначальной разблокировки.

Согласно поправке, токены будут освобождены от ограничений на перевод следующим образом:

* 30% с 1 декабря 2023 года;
* 40% в виде равных ежемесячных платежей с 1 января 2024 года по 1 июня 2024 года;
* 20% в виде равных ежемесячных платежей с 1 июля 2024 года по 1 июня 2025 года и
