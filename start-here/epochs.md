...
Все награды и контракты стейкинга функционируют на основе `28-дневных` циклов, называемых **эпохи**. Новая эпоха начинается автоматически после окончания текущей.
...
Для [пула стейкинга ликвидности](../staking-pools/liquidity-staking-pool.md) и [пула стейкинга безопасности](../staking-pools/safety-staking-pool.md) вводится график периодов вывода средств, чтобы обеспечить предсказуемость и регулярность доступности средств в пуле. Для вывода средств из стейка по окончании эпохи стейкер должен направить соответствующий запрос до начала периода недоступности вывода средств из стейка. В случае если от стейкера не поступает запрос на вывод размещенных в стейке средств, то они переносятся на следующую эпоху.
