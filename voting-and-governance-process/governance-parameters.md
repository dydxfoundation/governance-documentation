...
* [治理者](governance-parameters.md#governor-parameters)
* DYDX代币
* 资金库
* Merkle分配器
* 流动性质押
...
| 最低法定人数 | 通过提议所需的最少赞成票数 | 总供应量的2% | 总供应量的1% | 总供应量的10% | 总供应量的2% |
| 投票差值 | 通过提议所需的赞成-反对票数差值 | 总供应量的0.5% | 总供应量的0.5% | 总供应量的10% | 总供应量的0.5% |
| 投票时间 | 计票期间 | 4天 | 2天 | 10天 | 7天 |
| 时间锁延迟 | 执行通过并进入排队后，执行提议前的延迟 | 2天 | 0天 | 7天 | 2天 |
| 执行宽限期 | 投票后可执行提议的期限 | 7天 | 7天 | 7天 | 7天 |
| 时间锁最小延迟 | 执行提议前的最小延迟时间\（排队后\） | 1天 | 0天 | 5天 | 1天 |
| 时间锁最大延迟 | 执行提议前的最大延迟时间\（排队后\）
 | 7天 | 1天 | 21天 | 7天 |
...
| 所有者角色 | 可以更改战略/投票延迟/未经授权的执行器+拥有其他角色
 | 长时间锁 |
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td style="text-align:left">提议的创建和提议的投票之间的延迟(在区块中)
</td>
      <td
...
| 所有者 | 可以更新白名单；可以造币
 | 短时间锁 |
| 在转让受到限制之前 | 允许传输的最小时间戳\（以秒为单位\）
 | 中国时间7月14日晚上11:00 + 28天 + 7天 + 1天
 |
| 造币量的最大百分比
 | 代币所有者可造币量的最大百分比\(在造币限制结束后\)
 | 2 |
...
| 归属金额 + 预付金额
 | 即将立即发送的代币数量+归属于接收者的代币数量
 | 450,000,000 | 50,000,000 |
| 归属金额
 | 将分配给接收者的代币数量 | 374,616,438 | 50,000,000 |
| 预付金额 | 即将立即发送给接收者的代币数量 | 75,383,562 | 0 |
| 归属开始
 | 归属开始时间
 | 中国时间7月14日晚上11:00 | 中国时间7月14日晚上11:00 |
| 归属悬崖
 | 归属悬崖开始的时间\（并且代币实际上可以发送给接收者\）
 | 中国时间7月14日晚上11:00 | 中国时间7月14日晚上11:00 |
| 归属结束
 | 归属结束的时间
 | 中国时间7月14日晚上11:00 + 5年 | 中国时间7月14日晚上11:00 + 5年 |
| 资金库代理管理员
 | 可以升级资金库合约 | 短时间锁 | 短时间锁 |
| 资金库资金管理员
 | 可以使用资金库资金
...
| 最大的停市窗口期长度
 | 最大的可允许停市窗口期长度
 | 46天 |
| 每秒奖励 | 在分发窗口内每秒作为奖励发出的代币
 | 0.1585342262 = \(383,526 / \(28 \* 24 \* 60 \* 60\)\) |
| 借款人分配
 | 分配给每个借款人的资金百分比
 | 待定 |
| 所有者角色 | 以下所有角色的管理员
...
| 参数 | 说明 | 流动性质押
 |  |
| :--- | :--- | :--- | :--- |
| 所有者角色 | 拥有者、资金管理员和委托管理员角色+可以添加/删除接收资金的接收者+STARK密钥
 | 做市商 |  |
| 资金管理员角色 | 可以将超过借贷余额的资金提现给允许的接收者，并可以采取强制行动
 | 做市商 |  |
