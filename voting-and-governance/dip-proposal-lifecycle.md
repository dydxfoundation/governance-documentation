...
A continuación, describimos un borrador preliminar que explica cómo fluirá el proceso de gobernanza de dYdX v3, desde el inicio y la definición del concepto hasta la implementación. Esos procesos están sujetos a cambios según los comentarios de la comunidad de dYdX.
...
* un retraso de votación - un número de bloques de Snapshot que es de 6570 bloques (aproximadamente 1 día basado en un tiempo de bloque de 13,2 segundos) en el futuro. El número de bloque de Snapshot bloquea el estado de los miembros de la comunidad que pueden votar. Los titulares de tokens que poseen tokens antes del número de bloques de Snapshot son elegibles para votar. Antes de la captura del poder de voto respectivo de cada dirección, el retraso de la votación les da a los titulares de Tokens de gobernanza tiempo para adquirir tokens, delegar poder de voto y mover tokens entre billeteras. Ten en cuenta que el movimiento entre billeteras solo se aplica a $ethDYDX y $wethDYDX.
...
* el quórum mínimo: al menos `1 000 000` tokens de gobernanza. El quórum mínimo contribuye a la descentralización de la toma de decisiones y protege contra la toma de decisiones unilaterales y
...
dYdX Trading Inc. tendrá hasta 1 época (28 días), un período de gracia de ejecución, para implementar cambios basados en una encuesta de Snapshot exitosa.
...
## 6. Espera en la Cola y Ejecución
...
La facultad de voto en DYDX debe delegarse a una dirección ya sea completamente antes de que se haya presentado una propuesta o durante el **Retraso de la Votación**.
...
En el futuro, la gobernanza de dYdX puede votar para aumentar o disminuir **la demora en la votación**. Si bien hay beneficios obvios para un **Retraso en la votación.** Puede introducir algunos resultados adversos potenciales, como la explotación oportunista en casos extremos.

### ¿Cuál es el objetivo del umbral de la propuesta?

Dado que $ethDYDX y $wethDYDX son activos libremente negociables, cualquier persona puede intentar una toma de control de la gobernanza a través de la compra de mercado. Dicho esto, para forzar un voto de mala fe requeriría un mínimo de `5,000,000` Tokens de gobernanza en el caso de un bloqueo de corto tiempo; o `20,000,000` Tokens de gobernanza en el caso de un bloqueo de largo tiempo. Si no es totalmente imposible, esa cantidad sería prohibitivamente costosa y probablemente costaría más cuando se contabilizara la fluctuación de precios que la ganancia neta del ataque.
