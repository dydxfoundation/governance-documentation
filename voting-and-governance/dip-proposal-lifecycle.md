...
오프체인 DRC에 대한 대략적인 합의가 이루어지면, 거버넌스 토큰 `10,000` 이상의 총 제안권을 가진 커뮤니티 회원이 DRC에 대한 **오프체인 투표를** **스냡샷**으로 추진할 수 있습니다. 저희는 dYdX 커뮤니티가 정규 근무 주간의 가시성을 높이기 위한 스냅샷 여론조사를 월요일에 실시하도록 장려합니다.

스냅샷은 사용자가 오프체인으로 견해를 표명할 수 있는 간단한 투표 인터페이스입니다.
 스냅샷에서의 투표는 투표에 사용된 주소의 투표권에 따라 가중치가 부여됩니다.
...
제안된 변경 사항은 스냅샷 투표 결과가 다음을 충족할 경우, dYdX Trading Inc.가 실행하게 됩니다.
...
대략적인 합의에 도달하면 해당 유형의 제안에 대해 충분한 제안권을 보유한 커뮤니티 회원이 온체인 DIP를 제출할 수 있습니다.
...
| 매개 변수 | 설명 | 숏 타임락 실행자 | 머클-일시중지 실행자 | 롱 타임락 실행자 | 스타크웨어 실행자 |
| ------------------ | ------------------------------------------------ | ----------------------- | ---------------------- | ---------------------- | -------------------- |
...
DIP가 온체인으로 생성되면, 해당 DIP에는 **투표 딜레이**, **투표 기간**, **최소 정족수** 및 최소 **투표 격차**가 적용됩니다. 초기 매개 변수는 다음과 같습니다.
...
| 실행 유예 기간\* | 제안을 실행할 수 있으며 반드시 실행해야 하는 시간 | 7일 | 7일 | 7일 | 7일 |
...
$ethDYDX와 $wethDYDXrk는 자유롭게 거래할 수 있는 자산이므로, 모든 사람이 시장 매입을 통해 거버넌스에 시도할 수 있습니다. 즉, 부정 투표를 강제로 통과시려면 단기 시간 제한일 경우에는 최소 `5,000,000` 거버넌스 토큰이 필요하거나, 아니면 장기 시간 제한일 경우에 `20,000,000` 거버넌스 토큰이 필요합니다. 이를 달성한다 하더라도 이 금액은 매우 비쌀 것이며, 가격 변동을 고려할 때 공격을 통해 얻을 수 있는 순이익보다 더 많은 비용이 들 것입니다.


한 그룹이 어떻게든 악의적인 인수를 달성하더라도 타임락 딜레이를 통해 영향을 받은 에이전트는 프로토콜에서 자산을 인출할 시간을 확보할 수 있습니다. 이는 프로토콜을 포킹할 수 있는 기회이기도 하며, 이는 남아있는 신뢰 있는 참여자들이 취할 가능성이 높은 경로입니다.
