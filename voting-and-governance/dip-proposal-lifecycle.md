...
오프체인 DRC가 대략적인 합의에 도달하면 `10,000` DYDX 이상을 보유한 커뮤니티 회원은 **스냅샷**에서 DRC에 대해 **오프체인 투표**를 생성할 수 있습니다.
...
온체인 DIP가 생성되면, 해당 제안은 현재 `6570` 블록 또는 약 1일(블록당 13초 가정)에 해당하는 **투표 딜레이**로 정의되는 기간 동안 `보류` 상태에 진입합니다. 즉, DIP의 생성 1일 뒤 사용자 스냅샷이 촬영되며, 이때 해당 제안이 `활성` 상태로 전환됩니다.
